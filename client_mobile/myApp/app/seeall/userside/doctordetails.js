...
const DayCard = ({ day, isSelected, onToggle, availableTimes, selectedSlot, onTimeToggle }) => (
  <TouchableOpacity
    ...
    {isSelected && availableTimes && (
      <View style={styles.timeSlotsContainer}>
        {['morning', 'afternoon', 'evening', 'night'].map((period) => {
          const periodTimes = availableTimes.filter((slot) => slot.period === period);
                ...
                {periodTimes.map((slot) => (
                  <TouchableOpacity
                    ...
                    style={[
                      ...
                      selectedSlot?.slotId === slot.id && selectedSlot?.dayId === day.id && [
                  ...
                  >
                    <Text
                      style={[
                        styles.timeSlotText,
                        selectedSlot?.slotId === slot.id && selectedSlot?.dayId === day.id && styles.timeSlotTextActive,
                      ]}
                    >
                      {slot.hour}
                    </Text>
                    <Text
                      style={[
                        ...
                        selectedSlot?.slotId === slot.id && selectedSlot?.dayId === day.id && styles.timeSlotSubTextActive,
                      ]}
                    >
                      {slot.time}
                    </Text>
...
const DoctorProfile = () => {
  ...
  const fetchCounsellor = async () => {
    try {
      ...
      const url = `${API_BASE_URL}/api/users/counsellor?email=${encodeURIComponent(email)}`;
      ...
      if (json.success) {
            ...
            times.forEach((time) => {
              ...
              if (shiftId) {
                ...
                if (slot) {
                  ...
                  console.log(`No matching slot for time: ${time} in shift: ${shiftId}`);
                }
              } else {
        ...
        console.log('Initialized availability:', JSON.stringify(newAvailability, null, 2));
  ...
  const requestBooking = async () => {
    ...
    try {
      ...
      const dayFullName = weekDays.find((day) => day.id === selectedSlot.dayId)?.fullName;
      ...
      if (json.success) {
        Alert.alert('Success', 'Counsellor has been notified about your appointment request');
        ...
        Alert.alert('Error', json.message || 'Failed to create booking request');
      }
    } catch (err) {
    ...
    <SafeAreaView style={styles.container}>
      <ScrollView style={styles.scrollView} contentContainerStyle={styles.scrollContent}>
        ...
        <View style={styles.header}>
          ...
          <Text style={styles.title}>{counsellor?.fullName || doctorName || 'Doctor'}</Text>
        ...
        {loading ? (
          ...
          <View style={styles.loadingContainer}>
            ...
            <TouchableOpacity style={styles.retryButton} onPress={fetchCounsellor}>
              <Text style={styles.retryButtonText}>Retry</Text>
            </TouchableOpacity>
            <Text style={styles.fallbackText}>Showing sample data due to server error</Text>
            <View style={styles.profileSection}>
              <Image
                source={{ uri: `${API_BASE_URL}/Uploads/profile_photos/${dummyCounsellor.profilePhoto.filename}` }}
                style={styles.imagePlaceholder}
              />
            </View>
            <View style={styles.detailsSection}>
              <View style={styles.detailCard}>
                <MaterialCommunityIcons name="email" size={20} color="#007AFF" style={styles.detailIcon} />
                <View>
              ...
              <View style={styles.detailCard}>
                <MaterialCommunityIcons name="badge-account" size={20} color="#007AFF" style={styles.detailIcon} />
                <View>
                  <Text style={styles.detailLabel}>Designation</Text>
                  <Text style={styles.detailText}>{dummyCounsellor.designation}</Text>
                </View>
              </View>
              <View style={styles.detailCard}>
                <MaterialCommunityIcons name="currency-usd" size={20} color="#007AFF" style={styles.detailIcon} />
                <View>
                  <Text style={styles.detailLabel}>Charge per Hour</Text>
                  <Text style={styles.detailText}>Rs {dummyCounsellor.chargePerHour}</Text>
                </View>
              </View>
              <View style={styles.detailCard}>
                <MaterialCommunityIcons name="star" size={20} color="#FFD700" style={styles.detailIcon} />
                <View>
                  <Text style={styles.detailLabel}>Average Rating</Text>
                  <Text style={styles.detailText}>★ {dummyCounsellor.averageRating}</Text>
                </View>
              </View>
              {dummyCounsellor.bio && (
                <View style={styles.detailCard}>
                  <MaterialCommunityIcons name="information" size={20} color="#007AFF" style={styles.detailIcon} />
                  <View>
          ...
          <>
            ...
            <View style={styles.detailsSection}>
              ...
              <View style={styles.detailCard}>
                ...
                <View>
                  <Text style={styles.detailLabel}>Designation</Text>
                  <Text style={styles.detailText}>{counsellor.designation}</Text>
                </View>
              </View>
              <View style={styles.detailCard}>
                <MaterialCommunityIcons name="currency-usd" size={20} color="#007AFF" style={styles.detailIcon} />
                <View>
                  <Text style={styles.detailLabel}>Charge per Hour</Text>
                  <Text style={styles.detailText}>Rs {counsellor.chargePerHour}</Text>
                </View>
              </View>
              <View style={styles.detailCard}>
                <MaterialCommunityIcons name="star" size={20} color="#FFD700" style={styles.detailIcon} />
                <View>
                  <Text style={styles.detailLabel}>Average Rating</Text>
                  <Text style={styles.detailText}>★ {counsellor.averageRating || 0}</Text>
                </View>
              </View>
              {counsellor.bio && (
                <View style={styles.detailCard}>
              ...
              <View style={styles.infoCard}>
                ...
                <View style={styles.infoContent}>
                  ...
                  <Text style={styles.infoText}>
                    ...
                    • Choose one time slot for your appointment{'\n'}
                    • Each time slot is 1 hour long{'\n'}
                    • Provide notes and confirm to book the appointment
                  </Text>
                </View>
              </View>
                ...
                <View style={styles.modalOverlay}>
                  <View style={styles.modalContent}>
                    ...
                    <View style={styles.modalButtonContainer}>
                      ...
                      >
                        <Text style={styles.modalButtonText}>Request Booking</Text>
                      </TouchableOpacity>
                    </View>
                  </View>
                </View>
